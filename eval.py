...
from model.VQ_net import Model
...
def test_model(model, device, criterion, dataloaders):
    ...
    for ref, dis, labels in dataloaders[phase]:
        ...
        dis = dis.reshape(-1, dis.shape[2], dis.shape[3], dis.shape[4])

        with torch.no_grad():
            # preds, _ = model(ref, dis)
            preds = model(dis, ref-dis)
            preds = preds.flatten()
            preds = preds.sum(dim=0, keepdim=True)
...
if __name__=='__main__':
    ...
    dataloaders = {x: torch.utils.data.DataLoader(video_dataset[x], batch_size=1, shuffle=False, num_workers=0, drop_last=False) for x in ['test']}
